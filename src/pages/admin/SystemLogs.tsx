...
const SystemLogs = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const snap = await getDocs(query(collection(db, "system_logs"), orderBy("createdAt", "desc")));
  ...
  const filtered = logs.filter(l => !search || l.message?.toLowerCase().includes(search.toLowerCase()) || l.type?.includes(search));
    ...
    <div className="space-y-6">
      ...
      <Card><CardContent className="p-0">
        {loading ? <div className="flex justify-center py-12"><div className="h-6 w-6 animate-spin rounded-full border-4 border-primary border-t-transparent" /></div> : filtered.length === 0 ? (
          <p className="text-center text-muted-foreground py-12">No system logs</p>
        ) : (
          <Table>
            ...
            <TableBody>
              {filtered.map(l => (
                <TableRow key={l.id}>
                  <TableCell><Badge variant="outline" className={l.type === "error" ? "text-red-600" : l.type === "warning" ? "text-yellow-600" : ""}>{l.type}</Badge></TableCell>
                  <TableCell className="max-w-[400px] truncate text-sm">{l.message}</TableCell>
