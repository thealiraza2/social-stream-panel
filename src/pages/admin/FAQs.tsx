...
const FAQs = () => {
  ...
  const fetchData = async () => { setLoading(true); const snap = await getDocs(collection(db, "faqs")); setItems(snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a: any, b: any) => (a.sortOrder || 0) - (b.sortOrder || 0))); setLoading(false); };
  useEffect(() => { fetchData(); }, []);

  const handleSave = async () => {
    const data = { question: form.question, answer: form.answer, sortOrder: Number(form.sortOrder) };
    try {
      if (editing) await updateDoc(doc(db, "faqs", editing.id), data);
  ...
  const handleDelete = async (id: string) => { if (!confirm("Delete?")) return; await deleteDoc(doc(db, "faqs", id)); fetchData(); };

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">FAQs</h1>
        <Button onClick={() => { setEditing(null); setForm({ question: "", answer: "", sortOrder: "0" }); setDialogOpen(true); }} className="gradient-purple text-white border-0"><Plus className="mr-2 h-4 w-4" /> Add FAQ</Button>
      </div>
      <Card><CardContent className="p-0">
        {loading ? <div className="flex justify-center py-12"><div className="h-6 w-6 animate-spin rounded-full border-4 border-primary border-t-transparent" /></div> : (
          <Table>
            <TableHeader><TableRow><TableHead>Question</TableHead><TableHead>Order</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
            <TableBody>
              {items.map(i => (
                <TableRow key={i.id}>
                  <TableCell className="font-medium">{i.question}</TableCell>
                  <TableCell>{i.sortOrder}</TableCell>
                  <TableCell className="flex gap-1">
                    <Button size="icon" variant="ghost" onClick={() => { setEditing(i); setForm({ question: i.question, answer: i.answer, sortOrder: String(i.sortOrder || 0) }); setDialogOpen(true); }}><Pencil className="h-4 w-4" /></Button>
                    <Button size="icon" variant="ghost" className="text-destructive" onClick={() => handleDelete(i.id)}><Trash2 className="h-4 w-4" /></Button>
                  </TableCell>
                </TableRow>
              ))}
              {items.length === 0 && <TableRow><TableCell colSpan={3} className="text-center text-muted-foreground py-8">No FAQs</TableCell></TableRow>}
            </TableBody>
          </Table>
        )}
      </CardContent></Card>
